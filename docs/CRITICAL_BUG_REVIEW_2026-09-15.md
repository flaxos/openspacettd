...
| **OST-EC-001 P2, high** | Place conduit with catchment station but no loading service; expect honest delivery indication → `edge_conduit.cpp:238–243` ignores actual MoveGoodsToStation return while total_produced grows. Zero waiting is legitimate under service/rating rules, misleading counter/UAT is not. | Choose gross/allocated/buffered semantics explicitly; record actual delivery and expose reason/quantity. Station service/rating/competition/allocation tests and monthly waiting+onboard observation. UAT-05. Complete trace in master plan. |
